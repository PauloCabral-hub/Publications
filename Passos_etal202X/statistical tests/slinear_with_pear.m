% [alfa, beta,r, tcrit, tcalc, p] = slinear_with_pear(predictor,predicted, alpha, cook)
%
% DESCRIPTION: Performs a simple linear regression, gives the pearson correlation
...
%
% AUTHOR: Paulo Roberto Cabral Passos DATE: ??/??/????


function [alfa, beta,r, tcrit, tcalc, p, D] = slinear_with_pear(predictor,predicted, alpha, cook)
...
if cook == 1
    ...
    for o = 1:length(predictor)
        x = predictor(1, find(pos~=o) ); %#ok<FNDSB>
        y = predicted(1, find(pos~=o) ); %#ok<FNDSB>
        [b0, b1,~, ~, ~, ~, ~] = slinearwithpear(x,y, 0.05, 0);
