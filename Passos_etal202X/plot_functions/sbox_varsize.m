% sbox_varsize(group_id, data,  x_name, y_name, tit, box_names, sig_dif, test, acsis)
%
% DESCRIPTION: draw a boxplot identifierwise. Also use komolgorov-smirnov test to verify
...
%
% Comments: Graphical options parameters in the beggining of the function
...
for a = 1:max(auxgroup)
    ...
    plot(x,y, 'k','LineWidth',lwid)
    plot((a + 0.025*randn(1,length(find(group_id == a)))), data( find(group_id == a) ), 'ob', 'MarkerSize', msize, 'MarkerFaceColor', 'b') %#ok<FNDSB>
end
...
xlabel(x_name, 'FontSize',fsize, 'Interpreter', 'Latex')
ylabel(y_name, 'FontSize', fsize, 'Interpreter', 'Latex')
