         ...
         center_y = chan_imsites(b,2);
           for c =  (center_x-r_def):(center_x+r_def)
              for d = (center_y-r_def):(center_y+r_def)
                  idx_color = find(vcorr_pallet >= gchan,1);                  
                  if invert == 1
                      idx_color = length(vcorr_pallet)+1-idx_color;
                  end
                  overlayed_img(d,c,1) = pallet(idx_color,1);
                  overlayed_img(d,c,2) = pallet(idx_color,2); 
                  overlayed_img(d,c,3) = pallet(idx_color,3);                       
              end
           end
      end
   end
